...
__all__ = [
  ...
  # file upload helpers
  "UploadTestCase", "UploadRequest", "UploadFlaskClient", "httpfile",
  "tmphttpfile",
]
...
class TestCase(_TestCase, HttpAssertionsMixin):
  """
  ...
  def __call__(self, result=None):
    ...
    to call `super.setup()`.
    """
    self._app_ctx = self._request_ctx = self.app = None
    ...
    try:
      self._pre_setup()
      _TestCase.__call__(self, result)
    finally:
      self._post_teardown()
    if signals_available:
  ...
  def _pre_setup(self):
    ...
    # self._app_ctx.push()
    # self._request_ctx = self.app.test_request_context()
    # self._request_ctx.push()
  ...
  def get(self, handler, handler_args={}, *args, **kw):
    return self.open(
      handler, handler_args, method="GET", *args, **kw)

  def post(self, handler, handler_args={}, *args, **kw):
      return self.open(
        handler, handler_args, method="POST", *args, **kw)
...
def tmphttpfile(filename=None, data=None, size=None):
  """
  create a StringIO object to use with a flask test client.
